...
function ConsentModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-4 pb-6 md:items-center">
      <div className="w-full max-w-2xl rounded-3xl bg-card/95 p-6 shadow-2xl shadow-slate-950/40 backdrop-blur-sm">
        ...
        <div className="mt-5 grid gap-3">
          {(Object.keys(state) as Array<keyof ConsentState>).map((key) => (
            <label
              key={key}
              className="flex cursor-pointer items-center justify-between rounded-3xl border border-slate-800 bg-slate-950/90 px-4 py-4 transition hover:border-slate-700"
            >
              <div>
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-3">
          <button className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:border-slate-500" onClick={onClose}>
            Cancel
          </button>
          <button
...
export default function CookieConsent() {
  ...
  return (
    <>
      <div className="fixed inset-x-4 bottom-4 z-50 rounded-3xl border border-slate-800 bg-card/95 p-4 shadow-2xl shadow-slate-950/40 backdrop-blur-xl text-white md:left-8 md:right-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="font-semibold">We use cookies to improve your experience.</p>
            <p className="text-sm text-slate-400">Manage your preferences or accept all. Functional cookies are required for the site.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:border-slate-500" onClick={() => setModalOpen(true)}>
              Customize
            </button>
            <button className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:border-slate-500" onClick={rejectAll}>
              Reject All
            </button>
