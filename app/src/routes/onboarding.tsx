        ...
        if (data.available) {
          ...
          setError(
            data.reason === "reserved"
              ? "This slug is reserved. Contact support@koreshield.com to claim it."
  ...
  if (sessionPending) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center">
        <img src="/logo.png" alt="KoreShield" className="w-16 h-16 animate-pulse" />
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data) {
        ...
        navigate({ to: "/app/$orgSlug/install", params: { orgSlug: data.slug } });
      }
    } catch (err) {
      console.error(err);
    } finally {
