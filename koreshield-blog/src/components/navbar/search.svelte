...
const search = async (keyword: string, isDesktop: boolean): Promise<void> => {
    ...
    try {
        ...
        // If no results from Pagefind, show a helpful message
        if (searchResults.length === 0) {
            if (import.meta.env.DEV) {
                searchResults = fakeResult;
            } else {
                // In production without Pagefind, show a message about search
                searchResults = [{
                    url: "/archive",
                    meta: {
                        title: "Browse All Posts",
                    },
                    excerpt: `Try browsing our <a href="/archive">archive page</a> to find what you're looking for. Search functionality is enhanced in production.`,
                }];
            }
        }

        // Fallback for dev mode if no posts loaded
        if (import.meta.env.DEV && searchResults.length === 0) {
            searchResults = fakeResult;
        }
