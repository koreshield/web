...
| Endpoint | Method | Auth | Description |
|---|---|---|---|
| `/v1/chat/completions` | POST | Required | OpenAI-compatible chat endpoint — scan + forward + return |
...
KoreShield routes requests to the correct upstream provider based on the model name prefix — no extra configuration needed:
