...
All protected proxy endpoints use [SlowAPI](https://github.com/laurentS/slowapi) with Redis-backed storage. The default rate limit is `60/minute` and is configurable via `security.rate_limit` in `config.yaml`.
