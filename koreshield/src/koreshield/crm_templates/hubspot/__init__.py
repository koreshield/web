...
class HubSpotTemplate(CRMTemplate):
    ...
    def validate_configuration(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "api_key" in config:
            # For private app authentication, HubSpot recommends tokens starting with 'pat-'.
            # Other authentication methods (e.g., OAuth access tokens) may not use this
            # prefix.
            auth_type = config.get("auth_type")
            if auth_type == "private_app" and not config["api_key"].startswith("pat-"):
                errors.append(
                    "For HubSpot private app authentication, the API key should start with "
                    "'pat-'. "
                    "If you are using OAuth or another supported authentication method, "
                    "use the "
                    "appropriate token type and auth configuration instead of a private "
                    "app token."
                )
