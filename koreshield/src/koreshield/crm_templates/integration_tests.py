...
import pytest
from ..crm_templates import CRMTemplate, CRMPlatform, CRMSecurityPolicy
...
class CRMIntegrationTester:
    ...
    async def _mock_api_call(self, test_case: TestCase) -> Dict[str, Any]:
        ...
        # Get security policy and patterns
        policy = self.template.get_security_policy()
        attack_patterns = self.template.get_attack_patterns()
        pii_patterns = self.template.get_pii_patterns()
        business_rules = self.template.get_business_logic_rules()
