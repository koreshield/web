      ...
      POST: async ({ request, params }) => {
        ...
        try {
          const txtRecords = await resolveTxt(
            `_koreshield-challenge.${domain.domain}`,
          );
          ...
          if (!hasValidTxt) {
            return Response.json(
              {
                error: `TXT record verification failed. Expected "${domain.id}" at "_koreshield-challenge.${domain.domain}"`,
              },
              { status: 400 },
            );
          }
          ...
          try {
            ...
            cnameRecords.some((record) => record === "edge.koreshield.com");
          } catch (e) {
