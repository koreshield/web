      ...
      GET: async ({ request, params }) => {
        ...
        try {
          ...
          const avgDuration = parseFloat(
            (durationResult as any[])[0]?.avg_duration || "0",
          );
          ...
          const totalBandwidth = parseInt(
            (bandwidthResult as any[])[0]?.total_bytes || "0",
          );

          // Error rate
          const errorRateResult = await tigerQuery(
            `SELECT 
          ...
          const errorCount = parseInt((errorRateResult as any[])[0]?.errors || "0");
          const totalCount = parseInt((errorRateResult as any[])[0]?.total || "0");
          const errorRate =
          ...
          // Recent requests
          const requests = await tigerQuery(
            `SELECT 
               timestamp,
               method,
               path,
