    ...
    handlers: {
      GET: async ({ request, params }) => {
        ...
        try {
          ...
          const totalRequestsResult = await tigerQuery(
            `SELECT 
          ...
          const totalRequests = parseInt(
            (totalRequestsResult as any[])[0]?.total || "0",
          );

          // Requests from previous period
          const requestsYesterdayResult = await tigerQuery(
            `SELECT 
          ...
          const requestsYesterday = parseInt(
            (requestsYesterdayResult as any[])[0]?.total || "0",
          );
          ...
          const dataTransferResult = await tigerQuery(
            `SELECT 
          ...
          const totalBytes = Number((dataTransferResult as any[])[0]?.total || 0);

          // Data transfer (previous period)
          const dataYesterdayResult = await tigerQuery(
            `SELECT 
          ...
          const bytesYesterday = Number(
            (dataYesterdayResult as any[])[0]?.total || 0,
          );
          ...
          return Response.json({
            ...
            dataTransferChange,
            chartData: (chartData as any[]).map((d) => ({
              time: d.time,
